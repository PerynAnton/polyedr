...
class TestR3(unittest.TestCase):
    ...
    def test_dist01(self):
        b = R3(2.0, 0.0, 0.0)
        a = R3(0.0, 0.0, 0.0)
        self.assertAlmostEqual(a._dist_(b), 2)

    def test_dist02(self):
        b = R3(1.5, 0.0, 0.0)
        c = R3(-1.5, 0.0, 0.0)
        summ = 0
        if (c._is_good() and b._is_good()):
            summ += c._dist_(b)
        self.assertAlmostEqual(summ, 3)

    def test_is_good01(self):
        b = R3(1.5, 0.0, 0.0)
        self.assertAlmostEqual(b._is_good(), True)

    def test_is_good02(self):
        b = R3(2.0, 0.0, 1.0)
        self.assertAlmostEqual(b._is_good(), False)
